...
- 🔔 **Push, iMessage & desktop alerts** – When a session sits waiting for input, Posse sends a web-push notification to the paired phone, can text you over iMessage/SMS (`POSSE_IMESSAGE_TO`), and posts a native desktop notification with an OS-controlled sound. Desktop alerts are suppressed only while a focused Posse window is already showing the affected session, so background sessions still get your attention. ACP agents trigger the same desktop alert on structured `requestPermission` (decision needed) and `end_turn` (task completed) edges — never from assistant prose or tool-call completion.
...
- 🖥️ **Standalone terminal client** – The daemon serves its own terminal UI at `http://127.0.0.1:9811/terminal/`, usable while the Electron shell restarts. Open it from the standalone-terminal icon button in the sidebar top bar. It shares the app's mood color themes (Midnight / Dracula / Nord / Solarized / Monokai / Daylight) from a single source of truth (`src/shared/app-themes.ts`).
...
- 📄 **File preview & in-app edit** – Click any file in the directory tree to preview it in the right-side inspector: rendered Markdown, project-scoped HTML (scripts allowed, isolated from Node/Electron), images, SVG, Mermaid, CSV, PDF, and read-only Jupyter notebooks; everything else shows a syntax-highlighted source view (CodeMirror). Source view supports in-app editing with visible dirty state (`Save *`), conflict-aware saves (a write is blocked with a clear notice if the file changed on disk since you opened it), ⌘S/Ctrl+S, and an unsaved-changes guard when switching files. Binary or large files fall back to the external editor.
