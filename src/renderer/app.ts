...
function ensureFilePreview(): FilePreview {
  if (!filePreview) {
    filePreview = createFilePreview(filePreviewBody, {
      onSave: (path, content, expectedMtimeMs) => window.posse.writeFile(path, content, expectedMtimeMs),
...
async function openFilePreview(filePath: string, name: string): Promise<void> {
  ...
  // Path metadata: show the project-relative path so the user can tell identically-named
  // files apart; fall back to the absolute path's parent dir for files outside the root.
  const activeRoot = getActiveSessionCwd() || currentCwd;
  const rel = activeRoot ? relativePathWithinRoot(activeRoot, filePath) : null;
  const pathMeta = rel && rel !== name ? rel : filePath.replace(/\\/g, '/').split('/').slice(0, -1).join('/');
  filePreviewMeta.textContent = `${res.ext || 'txt'} · ${kb} KB · ${pathMeta}`;
  filePreviewMeta.title = filePath;
  ...
  ensureFilePreview().show(res.content || '', res.ext || '', filePath, htmlPreviewUrl, res.mtimeMs);
  setPreviewState('ready', '', '');
}
...
function destroySessions(ids: string[]): void {
  ...
  updateEmptyState();
  updateSessionTitleBar();
  removeSessionRowsInPlace(uniqIds);
  scheduleSessionChromeRender(Boolean(activeBefore && uniqIds.includes(activeBefore)));
}

let sessionChromeRenderScheduled = false;
let sessionChromeNeedsFileTree = false;

function removeSessionRowsInPlace(ids: string[]): void {
  const idSet = new Set(ids);
  for (const row of Array.from(sessionList.querySelectorAll<HTMLElement>('[data-session-id]'))) {
    if (row.dataset.sessionId && idSet.has(row.dataset.sessionId)) row.remove();
  }
}

function scheduleSessionChromeRender(refreshFileTree: boolean): void {
  sessionChromeNeedsFileTree ||= refreshFileTree;
  if (sessionChromeRenderScheduled) return;
  sessionChromeRenderScheduled = true;
  // Let Chromium paint the immediate row removal before doing the expensive full sidebar rebuild.
  requestAnimationFrame(() => {
    window.setTimeout(() => {
      sessionChromeRenderScheduled = false;
      const shouldRefreshFileTree = sessionChromeNeedsFileTree;
      sessionChromeNeedsFileTree = false;
      renderSessionList();
      if (shouldRefreshFileTree) void renderFileTree();
    }, 0);
  });
}
...
window.posse.onAcpStatus((id, info) => {
  ...
  if (info.status === 'closed' || info.status === 'error') {
    if (info.status === 'closed') {
      ...
      updateSessionTitleBar();
      removeSessionRowsInPlace([id]);
      scheduleSessionChromeRender(true);
    }
  }
});
...
(function initAppThemePicker() {
  ...
  const apply = (id: string): void => {
    ...
    document.documentElement.style.colorScheme = theme.id === 'daylight' ? 'light' : 'dark';
    ...
    document.dispatchEvent(new CustomEvent('posse:theme-changed', { detail: { id: theme.id } }));
  };
  if (select) {
    select.innerHTML = '';
